...
let notificationSoundEnabled = localStorage.getItem('notification_sound') !== 'false';
let browserNotificationsEnabled = localStorage.getItem('browser_notifications') !== 'false';
    ...
    if (notificationsList) {
        ...
        fetch('notification_api?action=get_notifications')
            ...
            .catch(error => {
                ...
                notificationsList.innerHTML = '<div class="empty-notifications">Error loading notifications</div>';
            });
    }
}
...
function createNotificationItem(notification) {
    ...
    if (notification.sender_pic) {
        ...
        placeholder.textContent = notification.sender_name ? notification.sender_name.charAt(0) : 'C';
...
function checkNotifications() {
    fetch('notification_api?action=get_count')
        .then(response => response.json())
        .then(data => {
            if (data.success) {
...
function updateNotificationBadge(count) {
    if (notificationBadge) {
        if (count > 0) {
            ...
            // Show browser notification
            if (browserNotificationsEnabled && Notification.permission === 'granted') {
                showBrowserNotification('Cupid', 'You have ' + count + ' new notification(s)');
            }
        } else {
